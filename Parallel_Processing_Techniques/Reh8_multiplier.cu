...
// Device function for 2-bit Reh multiplication
__device__ uint8_t Reh2_mult(uint8_t a, uint8_t b) {
    uint8_t Y = 0;
    
    // Y[0] = (a[0] & b[1]) & (a[1] & b[0])
    Y |= ((a & 1) & ((b >> 1) & 1)) & (((a >> 1) & 1) & (b & 1));
    
    // Y[1] = (a[0] & b[1]) ^ (a[1] & b[0])
    Y |= (((((a & 1) & ((b >> 1) & 1)) ^ (((a >> 1) & 1) & (b & 1))) & 1) << 1);
    
    // Y[2] = ((a[0] & b[1]) & (a[1] & b[0])) ^ (a[1] & b[1])
    Y |= ((((a & 1) & ((b >> 1) & 1)) & (((a >> 1) & 1) & (b & 1)) ^ (((a >> 1) & 1) & ((b >> 1) & 1))) & 1) << 2;
    
    // Y[3] = (a[0] & b[1]) & (a[1] & b[0])
    Y |= ((((a & 1) & ((b >> 1) & 1)) & (((a >> 1) & 1) & (b & 1))) & 1) << 3;
    
    return Y;
}

// Device function for 4-bit Reh multiplication
__device__ uint16_t Reh4_mult(uint8_t a, uint8_t b) {
    ...
    uint8_t AL_BL = Reh2_mult(aL, bL);
    uint8_t AH_BL = Reh2_mult(aH, bL);
    uint8_t AL_BH = Reh2_mult(aL, bH);
    uint8_t AH_BH = Reh2_mult(aH, bH);
...
// Device function for 8-bit Reh multiplication
__device__ uint32_t Reh8_mult(uint8_t a, uint8_t b) {
    ...
    uint16_t AL_BL = Reh4_mult(aL, bL);
    uint16_t AH_BL = Reh4_mult(aH, bL);
    uint16_t AL_BH = Reh4_mult(aL, bH);
    uint16_t AH_BH = Reh4_mult(aH, bH);
...
// Kernel function using Reh8 multiplication
__global__ void multiply_kernel(NumberPair* pairs, uint16_t* results, int n) {
    ...
    if (idx < n) {
        ...
        results[idx] = static_cast<uint16_t>(Reh8_mult(pairs[idx].a, pairs[idx].b));
    }
}
...
int main() {
    ...
    std::ofstream outFile("./output/output_from_multiplier_from_cuda_Reh8.dat");
    if (!outFile) {
